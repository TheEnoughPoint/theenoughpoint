...
takeaways:
  ...
  - "The entry gap over nearby resale is <strong>not one number</strong>: on the same price list it runs <strong>+18% to +76%</strong> depending purely on the size of the unit."
...
**Where S$2,900 comes from, since it is doing a lot of work.** It is not a guess and it is not anyone's quote. Two sections down we take the land cost — S$1,178 per square foot per plot ratio, verified at the marketing agent's own award announcement — and check it against eight recent sites where the winning bidder's registered entity matches the developer now selling on that land. Those pairs put today's asking price at 2.13 to 2.83 times the land rate, median 2.31. Applied to S$1,178 that implies roughly **S$2,500 to S$3,300**, with the median of the pairs landing near S$2,722. S$2,900 sits inside that range and a little above its middle. It is a working figure for showing you the shape of the arithmetic — not a prediction of what October will say, and the whole point of what follows is that you re-run it on the real number.
...
| Unit size | District 20 resale median | At S$2,600 | At S$2,900 | At S$3,100 |
|---|---|---|---|---|
...
The spread is the useful part. One price list, one building, one morning — and the gap over the surrounding market runs from **+18% to +76%** depending only on which door you walk through. Mid-size units are the tightest-priced against their local comparison; the largest are the most stretched, partly because the large-unit band here is weighted towards much older stock, which drags the comparison down and overstates the true gap at the top end.
...
**Take the price list to your own size band**, not the district median. The gap on a 700 square foot unit and a 1,300 square foot unit in the same building are different numbers.
...
- **The land multiple is eight pairs.** All state land sales, all awarded 2022 to 2024, and the launch side is each project's *current* median asking price rather than its launch-day price. Both sides of the Thomson Reserve comparison are today's prices, so the comparison is like for like, but the ratio should not be read as a margin. It also embeds market movement between award and today. Restating every pair at the price index of its own award quarter pulls the median from 2.31× to **2.11×** — still nowhere near the old rule. A single flat haircut using the 6.5% two-year index move gives about 2.16×, but that understates the older sites: the 2022 award in the table saw the index move 16.3%, not 6.5%.
